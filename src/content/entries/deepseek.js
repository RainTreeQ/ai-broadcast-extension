    ...
    if (message.type === MESSAGE_TYPES.INJECT_IMAGE) {
      ...
      (async () => {
        ...
        try {
          ...
          sendResponse({ success: result.success, platform: adapter.name, strategy: result.strategy, totalMs, error: result.success ? undefined : '图片粘贴失败' });
        } catch (err) {
